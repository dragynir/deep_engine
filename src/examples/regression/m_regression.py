...
def visualize_decision(X, y, pred, name, sampled_x=None, sampled_y=None, params=""):
    if VIS_LINE:
        plt.plot(X, y)
        plt.plot(X, pred)
    else:
    ...
    if sampled_x is not None:
        plt.scatter(sampled_x, sampled_y, color="r")
...
def create_custom_datasets(
    dataset_name="cos", dots=100, noise_level=0.0, noise_type="uniform"
):
    """Cos, poly, sin"""
...
if __name__ == "__main__":
    ...
    # n_elements x 1
    # X_full_origin, y_full = create_custom_datasets(dataset_name='poly', dots=100)
    # X_full_origin, y_full = create_custom_datasets(dataset_name='sin', dots=100)
    # X_full_origin, y_full = create_custom_datasets(dataset_name='sin', dots=100, noise_level=0.2, noise_type='uniform')
    X_full_origin, y_full = create_custom_datasets(
        dataset_name="sin",
        dots=100,
        noise_level=0.2,
        noise_type="normal",
    ...
    for poly_degree in poly_grid:
        for alpha in alpha_grid:
            ...
            X_full = np.concatenate(
                [X_full, np.ones(X_full.shape[0])[..., None]], axis=-1
            )
            ...
            if calc_mode:
                ...
                if cross_val_mode:
                    for fold_i, (val_fold, train_fold) in enumerate(
                        ...
                        visualize_decision(
                            X_fold[:, 0], y_fold, y_fold, name="fold_data"
                        )
                        model.optimize(
                            X_origin,
                            X_fold,
